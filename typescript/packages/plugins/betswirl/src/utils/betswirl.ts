...
export async function getBetToken(
    ...
    if (
        ...
        if (!selectedToken) {
            throw new Error(
                `The token must be one of ${casinoTokens
                    .map((casinoToken) => casinoToken.symbol)
                    .join(", ")}`
            );
        }
    } else {
...
async function getBetRequirements(
    ...
    try {
        ...
        throw new Error(
            `An error occured while getting the bet requirements: ${error}`
        );
    }
}
    ...
    try {
        ...
        */
        ...
        const response = await fetch(
            `https://api.betswirl.com/vrfFees?${params}`,
            {}
        );
